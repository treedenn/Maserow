...
import me.heitx.maserow.common.database.repository.ISmartScriptRepository;
...
import me.heitx.maserow.common.utils.QueryUtil;
...
public class SmartScriptRepository extends MySqlDatabase implements ISmartScriptRepository {
	...
	public List<SimpleSearchModel> search(long sourceType, long entryOrGuid, String name, boolean withExisting) {
		...
		execute(Database.Selection.WORLD, conn -> {
			...
			if(withExisting) {
				// Entry is editor, which is why only one should be added.
	...
	@Override
	public List<SmartScript> getAll(long entryOrGuid, int sourceType) {
		AtomicReference<List<SmartScript>> atomic = new AtomicReference<>(new ArrayList<>());

		execute(Database.Selection.WORLD, conn -> {
			Statement ps = conn.createStatement();

			List<String> andBlocks = new ArrayList<>();
			andBlocks.add("entryorguid = " + entryOrGuid);
			andBlocks.add("source_type = " + sourceType);

			List<String> blocks = QueryUtil.select(null, "smart_scripts", QueryUtil.and(andBlocks));

			final ResultSet rs = ps.executeQuery(QueryUtil.buildNewLineFormat(false, blocks));
			atomic.set(convertRows(SmartScript.class, rs));
