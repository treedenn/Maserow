...
import me.heitx.maserow.common.database.repository.IMailRepository;
...
//					}
	...
	private void buttonSendAction(ActionEvent event) {
		if(Database.hasAccess(Database.Selection.WORLD) && Database.hasAccess(Database.Selection.CHARACTERS)) {
			...
			IMailRepository repo = Database.getInstance().getMailRepository();
			repo.send(getMail(), items, receivers, races, classes);
		}
	}
